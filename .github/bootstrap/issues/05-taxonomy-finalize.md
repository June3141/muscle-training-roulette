...
---
> [!NOTE]
> **完了（2026-08-17）。** 残っていた 3 つの論点をすべて決めた。分類は **22** になった。
>
> | 論点 | 結論 |
> |---|---|
> | **ローテーターカフ** | 分類を足さず、純粋な 5 件を `selectable: false` に。深層筋で人体図に描けず、分類を足しても目盛りが増えるだけ。Face Pull など残り 4 件は三角筋として写せた |
> | **手関節屈筋** | **`wrist_flexors` を追加。** 21 件のためなら見送りだったが、実際は**握力補助 76 件を含む 94 件**が写る先を必要としていた。腕橈骨筋は肘屈曲筋なので握力を表現できない |
> | **中臀筋の配分** | 片脚支持 / ワイドスタンス / 前額面の動作の 3 ルールで **45 件に補完**。上流の 5 件から 50 件規模になり部位として機能する |
